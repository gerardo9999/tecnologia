...
class ctrlOrdenAtencion extends Controller
{
    public function mostrarOrdenes(Request $request){
        ...
        if($buscar==''){
             ...
             $orden= ordenAtencion::join('cliente','cliente.id','=','ordenatencion.idCliente')
             ->select(
                 'ordenatencion.idCliente',
             'ordenatencion.id',
             DB::raw('CONCAT(nombres," ",apellidos) as nombreCompleto'),
             'ordenatencion.montototal',
             'ordenatencion.fecha'
             )
             ->orderBy('ordenatencion.id','desc')
             ->paginate(10);
         }
         else{
            $orden= ordenAtencion::join('cliente','cliente.id','=','ordenatencion.idCliente')
            ->select('ordenatencion.idCliente',
            'ordenatencion.id',
            DB::raw('CONCAT(nombres," ",apellidos) as nombreCompleto'),
            'ordenatencion.montototal',
            'ordenatencion.fecha')
    ...
    public function itemEliminar(Request $request){
        ...
        $detalleOrden->delete();   
        $bitacora = bitacora::guardar('ordenAtencion','eliminar',$ordenAtencion->id);
